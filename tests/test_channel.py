...
def test_build_telegram_channel(event_loop):
...
def test_fetch_send_banner_on_startup_from_url(event_loop):
...
def test_unknown_channel_type_raises_error(event_loop):
